...
public class Board {
    ...
    public int manhattan() {
        ...
        for(int x = 0; x < nx; x++) {
            for(int y = 0; y < ny; y++) {
                ...
                if(actual.x == getTargetPosition(x,y).x && actual.y == getTargetPosition(x,y).y) {}
                else if(field[x][y] == 0) {}
                else {
                    count = count + Math.abs((actual.x - getTargetPosition(x,y).x));
                    count = count + Math.abs((actual.y - getTargetPosition(x,y).y));
                }
            }
        }
        return count;
    ...
    public Position getTargetPosition(int x, int y) {
        Position distance = new Position((field[x][y]-1)/nx, (field[x][y]-1)%ny);
        return distance;
    }
